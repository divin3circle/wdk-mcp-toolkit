...
## 🤖 Agent Control Tools (Autonomous Mode)

For multi-tenant autonomous agents, this package now includes adapter-oriented MCP tools for wallet config metadata, goals/policies, and scheduling.
...
These tools are designed to complement (not replace) your policy engine and signer service.

- `CUSTODY_TOOLS`: creates/reads wallet config metadata in `~/.wallets/wallets.json`; key material is encrypted at rest
- `GOVERNANCE_TOOLS`: stores goals and policy in `~/.wallets/governance.json` and analyzes goals against policy constraints
- `SCHEDULER_TOOLS`: creates/updates/deletes/lists scheduled tasks and calls `openclaw cron` commands

Scheduler naming compatibility:

- Canonical names: `addCronJob`, `editCronJob`, `removeCronJob`
- Legacy aliases (still supported): `createCronJob`, `updateCronJob`, `deleteCronJob`

Required environment variable for custody encryption:

```bash
export WDK_WALLET_ENCRYPTION_KEY="your-strong-random-secret"
```

Required environment variable for agent-initiated writes and CLI mutations:

```bash
export WDK_ALLOW_AGENT_MUTATIONS=1
```

Enforcement behavior:

- If `WDK_ALLOW_AGENT_MUTATIONS` is not `1`, agent-initiated wallet writes and scheduler CLI mutations are blocked.
- Scheduler CLI args are validated with an allowlist per verb (`openclaw cron add|edit|remove`) before command execution.
- All successful mutations are appended to `~/.wallets/audit.log`.

## 🧩 Using with OpenClaw (Local Gateway)

If users already run OpenClaw on their machine, they can register this toolkit as a local MCP stdio server.
...
This generates an OpenClaw config snippet at `~/.openclaw/wdk-mcp-toolkit.mcp.json5` you can merge into `~/.openclaw/openclaw.json`.
...
2. Add a stdio MCP server to your OpenClaw config (`~/.openclaw/openclaw.json`).
...
```json5
{
  tools: {
    mcpServers: {
      wdk: {
        command: "node",
        args: ["/absolute/path/to/wdk-mcp-toolkit/examples/agent/index.js"],
        env: {
          WDK_SEED: "your twelve word seed phrase here",
          WDK_WALLET_ENCRYPTION_KEY: "your-strong-random-secret",
          WDK_ALLOW_AGENT_MUTATIONS: "1",
          WDK_INDEXER_API_KEY: "optional",
          MOONPAY_API_KEY: "optional",
          MOONPAY_SECRET_KEY: "optional",
        },
      },
    },
  },
...
3. Create policy before scheduler writes:

- Call `setAgentPolicy` once for each `ownerId`
- Then use `addCronJob` / `editCronJob` / `removeCronJob`

> **📁 Example:** Use [`examples/agent/index.js`](examples/agent/index.js) for OpenClaw integration. It includes wallet, pricing, indexer, swap, bridge, lending, fiat, custody, governance, and scheduler tools.
